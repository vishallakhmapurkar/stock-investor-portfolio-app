    ...
    $.unblockUI()
		  ...
		  $('#email').blur(function() {
			 ...
			 $('#username').blur(function() {
                if ($('#username').val()) {
				   ...
				   userAlreadyRegistered($('#username').val());
                }
            });
        ...
        success: function(data) {
        	...
        	if(data=='fail'){
        		...
        		msg = 'Thank You For registeration. Your password has been sent on your email id '+data+ ', Please check ur email.\n<a href="login.do">Click Here to Login</a>';
        	}
        	$("#box1").remove();
        	$("#box2").remove();
        	$("#tab1").slideUp('slow',function() {$("#tab1").remove();});
        ...
        success: function(data) {
			...
			function userAlreadyRegistered(user){
			   $.ajax({
        url: 'userNameExist.do',
        type: 'post',
        data: $.param({user:user}),
        success: function(data) {
        	if(data=='fail'){
			jAlert('User Id already exist: ', 'Duplicate User error');
			$('#email').val('');
			 $('input:submit').attr("disabled", true);
			}else{
			 $('input:submit').attr("disabled", false);
	...
	<div class="center">
		...
		<div id="midd"
			...
			<div id="menuActive" style="width: 100%; margin-top: -5%;">
				<ul id="tabs">
					<li><a href="#" name="tab1">Money Tracker Investor  Registration Form</a></li>
...
<input type="submit" class="buttons" id="step2"
								value=" Submit Your Details " style="cursor: pointer" />
								</fieldset>
							</div>
							&nbsp; &nbsp; 
